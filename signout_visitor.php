...
<title>:: FRSC :: Today's Logbook Record</title>
...
$fetch="SELECT * FROM `logbook` WHERE `signin`= 1 ORDER BY `timein` ASC LIMIT $start, $display ";
$fetch=@mysql_query($fetch) or die(mysql_error());
if($object->checkNumofVisitors($fetch)==false){echo "<p class='msg warning'>All Visitors have been signed out!</p>";}
...
?>
  ...
  <h1>SIGN OUT GUEST</h1>
  <p> Click the link to signout a visitor</p>
  <?php
  
  if($_GET['signout']=="true"){
	  echo "<p class='msg done'>Visitor has been signed out successfully!</p>";
	  }
	  ?>
...
<td><?php echo "<a href='signout.php?vid={$row['id']}'> Signout Guest</a>";?>
</td>
</tr>
...
if ($current_page != 1) {
 echo '<center><a href="signout_visitor.php?s=' .
...
for ($i = 1; $i <= $pages; $i++) {
 if ($i != $current_page) {
 echo '<a href="signout_visitor.php?s=' .
(($display * ($i - 1))) . '&p=' .
$pages . '">' . $i . '</a> ';
 } else {
 echo $i . ' ';
